...
try:
    ...
    print()
    print("  Please run:")
    print("  pip install flask flask-login werkzeug python-dotenv")
    print("=" * 60)
    print()
...
MAX_CONTENT_LENGTH = int(os.environ.get("MAX_UPLOAD_GB", 10)) * 1024 * 1024 * 1024
...
APP_USERNAME = os.environ.get("APP_USERNAME", "dov")
_raw_pw = os.environ.get("APP_PASSWORD", "yunis2026")
        ...
        for letter in string.ascii_uppercase:
            ...
            if os.path.exists(drive_path):
                if ALLOWED_DRIVES and letter not in ALLOWED_DRIVES:
                    continue
                try:
                    total, used, free = _shutil.disk_usage(drive_path)
                    drives.append({
                        "letter": letter,
                        "path": drive_path,
...
if __name__ == "__main__":
    ...
    print(f"  Mode:  {BROWSE_MODE}")
    if BROWSE_MODE == "drives":
        print("  Drive browsing enabled")
    print()
    print("  For public access, open another CMD and run:")
